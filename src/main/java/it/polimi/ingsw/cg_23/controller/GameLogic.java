...
public class GameLogic implements Observer {
	...
	public boolean validMove(Player player, Sector destination) {
		...
		if (player instanceof Alien && ((Alien) player).getHasKilled()) {// three step for aliens after killing someone
			for (Sector sector : player.getCurrentSector().getNeighbors()) {
	...
	/**
	 * First of all the method creates an iterator containing the players in the
	 * same sector of the attacker. <br>
	 * The while statement iterates until the player list has a next element. <br>
	 * In the while statement the method check if the attacked player is
	 * different from attacker and the attacked hasn't <br>
	 * the defense card. If the condition of the if statement is verified he set
	 * as dead the attacked player, remove from <br>
	 * player list and call a method that removes the dead player from the list
	 * of player in the match. <br>
	 * If the condition in if statement is not verified, calls another if that
	 * check if the player has a defense card and uses it.
	 * 
	...
	/**
	 * This card can not be used from human, but auto-used when human is
	 * attacked, to prevent him to be killed.
	 */
	...
	/**
	 * This method set as true human attribute escaped. Set this escape hatch
	 * sector as unusable.
	 */
	 ...
	 * After that the controller check if in the selected sector and in the
	 * nearby there are someone. <br>
	 * If this sector aren't empty the model notify the view that communicate
	 * the position of this players.
	 */
	 ...
	 * We check if the card player want to use is the same type of a card he has
	 * in his hand.
	 * 
	 * @param player
	 * @param card
	 ...
	 * First of all we check if player has the card he want to use (calling
	 * hasCard() method), <br>
	 * if he has it calls the method doAction in the card.<br>
	 * When the card has finished to do its action, the method calls discardCard
	 * to put the card in the right discard <br>
	...
	public void choseHowUseItemCard(Player player, Card itemCard, String choice) {
		...
		if (player.getCards().size() > 3) {
			if (choice != null) {
				...
				// TODO chiedere alla view di chiedere al giocatore cosa vuole
				// fare e quale carta vuole usare o buttare
				// quindi c'è da aggiungere la scelta della carta tra quelle che
				// ha in mano con un for o cacchio ne so!
				switch (choice) {
				case "usa":
